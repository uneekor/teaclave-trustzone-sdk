...
fn update(session: &mut Session, src: &[u8]) -> optee_teec::Result<()> {
    ...
    session.invoke_command(Command::UpdateDigest as u32, &mut operation)?;
    Ok(())
}
...
fn reset_key(session: &mut Session) -> optee_teec::Result<()> {
    let mut operation = Operation::new(0, ParamNone, ParamNone, ParamNone, ParamNone);
    session.invoke_command(Command::ResetKey as u32, &mut operation)?;
    Ok(())
}
...
fn main() -> optee_teec::Result<()> {
    ...
    if args_len == 2 && args[1] == "--reset" || args[1] == "-r" {
        println!("kaboom! reset key");
        reset_key(&mut session)?;
        return Ok(());
    }
    ...
    println!(
        "Get message hash as: {:?}. (hash length {})",
        hex::encode(&res),
        hash_length
    );
